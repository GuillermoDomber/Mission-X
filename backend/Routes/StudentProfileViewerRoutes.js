...
router.get("/studentprofileviewer", (req, res) => {
    ...
    //   if (err) return console.log("something is wrong",err);
      pool.query("SELECT teacher.name AS teacher_name,student.student_id,student.name,student.teacher_id,course,student.email,student.date_of_birth,student.contact_number, student.profile_pic FROM student LEFT JOIN teacher ON student.teacher_id = teacher.teacher_id where student_id=54", (err, result) => {
        if (err) return console.log("something is wrong",err);
